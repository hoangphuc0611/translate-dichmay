...
vocab_ipt, vocab_opt = pickle.load(open("./vocab_trans.pkl", 'rb'))

encoder_maxlen = vocab_ipt.maxlen+2
decoder_maxlen = vocab_opt.maxlen+2
    ...
    for x in text.split():
        ...
        if x not in vocab_ipt.w2i.keys() and x!=',':
          ...
          for y in text[index:].split(' , '):
            child.append(predict(y))
          results+=', '.join(child)
    return re.sub('\s+', ' ', results).replace(' , ',', ')
...
def check_tweet():
    if request.method=='POST':
        ...
        return render_template('index.html',data=evaluate(tweet),tweet=tweet)
